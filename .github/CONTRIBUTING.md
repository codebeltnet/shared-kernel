...
- The solution is `Codebelt.SharedKernel.slnx`. Central package versions are maintained in `Directory.Packages.props`.
...
- `Codebelt.SharedKernel.slnx` is the solution used for local development.
...
```powershell
dotnet restore "Codebelt.SharedKernel.slnx"
dotnet build "Codebelt.SharedKernel.slnx" --configuration Release --no-restore
```
...
```powershell
dotnet pack "Codebelt.SharedKernel.slnx" --configuration Release --no-restore
```
...
By contributing to Shared Kernel API by Codebelt, you agree that your contributions will be licensed under the MIT license.
